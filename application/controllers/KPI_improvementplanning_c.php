...
class KPI_improvementplanning_c extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('id') == NULL) {
            ...
            //mengambil list seluruh departemen
            $this->load->model('Departemen_m');
            $this->datakirim['departemen'] = $this->Departemen_m->kpiImprovementPlanningProgress($year);

            $this->load->view('KPI_improvementplanning_v', $this->datakirim);
        }
    }

    public function viewPlanning($idDepartemen, $tahun) {
        //echo $tahun;
        $this->load->model('Departemen_m');
        ...
        $this->load->model('Improvement_m');
        $this->datakirim['improvement'] = $this->Improvement_m->getImprovementKPI($idDepartemen, $tahun);


        $this->load->view('KPI_improvementplanning_vp_v', $this->datakirim);
    }
    ...
    public function cetakPDF($idDepartemen, $tahun) {
        ...
        $this->fpdf->Cell(20, 10, "List Rencana Improvement Periode Th. $tahun", 0, 0, 'C');
        $this->fpdf->Ln(10);
